package com.example.course.controller.mysql.contract;
/*
...
@Controller
@RequestMapping("/ui/v1/mysql/contracts")
public class MySQLContractUIController {
    ...
    public String showAll(Model model){
        List<MySQLContract> list = contractService.getAll();
    ...
    public String delete(@PathVariable Long id){
        MySQLGroup group = groupService.getByName(contractService.get(id).getGroup().getName());
        ...
        return "redirect:/ui/v1/mysql/contracts/";
    }
    ...
    public String addContract(Model model){
        ...
        List<MySQLStudent> students = studentService.getAll();
        List<MySQLGroup> groups = groupService.getAll();
        for (MySQLStudent student: studentService.getAll()) {
            for (MySQLContract contract: contractService.getAll()) {
        ...
        for (MySQLGroup group : groupService.getAll()){
            if(group.getAmountOfStudents() == 20 || group.getEducationStatus() != EducationStatus.ОЧІКУЄТЬСЯ){
                groups.remove(group);
            }
        }
    ...
    public String addContract(@ModelAttribute("contract-form") ContractForm contractForm,
        ...
        MySQLContract contract = new MySQLContract();
        contract.setName(contractForm.getName());
        contract.setDescription(contractForm.getDescription());
        contract.setStudent(studentService.get(Long.valueOf(contractForm.getStudent().split(": ")[0])));
        MySQLGroup group = groupService.getByName(contractForm.getGroup().split(", ")[0]);
        ...
        return "redirect:/ui/v1/mysql/contracts/confirm/"+contract.getId();
    }
    ...
    public String addNewStudent(@ModelAttribute("student-form") NewStudentContract studentContract,
                                @RequestParam(value = "postponeCheck", defaultValue = "false") boolean postponeCheck,
                                @RequestParam(value = "privilegeCheck", defaultValue = "false") boolean privilegeCheck){
        MySQLStudent student = new MySQLStudent();
        ...
        MySQLGroup group = groupService.getByName(studentContract.getGroup().split(", ")[0]);
        contract.setGroup(group);
    ...
    public String confirmContract(Model model, @PathVariable Long id){
        MySQLContract contract = contractService.get(id);
    ...
    public String editContract(Model model, @PathVariable("id") Long id){
        MySQLContract contract = contractService.get(id);
        ContractForm contractForm = new ContractForm();
        contractForm.setId(contract.getId().toString());
    ...
    public String addNewLanguage(Model model, @PathVariable Long id){
        ContractForm contractForm = new ContractForm();
        List<MySQLGroup> groups = groupService.getAll();
        MySQLStudent student = studentService.get(id);
    ...
    public String addNewLanguage(@ModelAttribute("contract-form") ContractForm contractForm, @PathVariable Long id){
        MySQLContract contract = new MySQLContract();
        contract.setName(contractForm.getName());
        contract.setDescription(contractForm.getDescription());
        ...
        //доробити у формі
        payment.setPostponePayment(false);
        payment.setPayTo(payment.isPostponePayment() ? contract.getGroup().getStartDate().plusMonths(1)
                : contract.getGroup().getStartDate());
        payment.setPrivilege(false);
        payment.setTotalAmount(payment.getContract().getGroup().getTotalPrice());
        payment.setPaymentStatus(payment.getTotalAmount() <= payment.getPaidAmount() ? PaymentStatus.ОПЛАЧЕНО :
                payment.getTotalAmount() > payment.getPaidAmount() && payment.getPaidAmount() != 0 ? PaymentStatus.ЧАСТКОВО :
                        PaymentStatus.ОЧІКУВАННЯ);
